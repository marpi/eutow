var City = function () {
    ...
    function reload() {
        ...
        var reflectionCube = Assets.getCubeMap(10)
        reflectionCube.format = THREE.RGBFormat;

        var cubeWidth = 15000;
        var numberOfSphersPerSide = 40;
        ...
        for (var alpha = 0, alphaIndex = 0; alpha <= 1.0; alpha += stepSize, alphaIndex++) {
            for (var gamma = 0; gamma <= 1.0; gamma += stepSize) {
                ...
                mesh.scale.y = 1 + Math.random() * 3 * 2
                mesh.scale.x = mesh.scale.z = 1 + Math.random() * 1

                mesh.position.x = alpha * cubeWidth - cubeWidth / 2 - stepSize * cubeWidth * Math.random();
                mesh.position.y = -800 + sphereRadius * mesh.scale.y / 2
